...
// These copies are recorded on the existing FG UI list/queue. The provider is
// tagged ValidNow, so later consumption uses its own frame-owned copy.
void IFGFeature_Dx12::RetireGuide(ID3D12Resource*& resource)
{
    if (!resource) return;
...
void IFGFeature_Dx12::CollectGuides()
{
...
void IFGFeature_Dx12::SetPresentationGuideDelay(int age)
{
...
bool IFGFeature_Dx12::MatchPresentationGuide(Dx12Resource& input, int index)
{
    ...
    if (kind == 1 && (_frameCount % 240 == 0 || age == 0))
        LOG_INFO("{} matched NR guides: frame={} scene={} age={} render={}x{}", Name(), _frameCount,
