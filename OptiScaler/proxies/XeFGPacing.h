...
inline void TryPace(void* ctx, void* arg5, void* arg6, uint64_t arg7, bool isLast)
{
    ...
    // Keep pacing coverage consistent with the configured unlock ceiling.
    if (count < 1 || count > Config::XeFGMaxInterpolations)
        return;
...
inline void* TsDetour(void* a1, int64_t* out, void* lookup, void* timing, uint32_t index, uint32_t countPlus1)
{
    ...
    // integration caps the multiplier at the shared unlock ceiling.
    if (out == nullptr || timing == nullptr || index == 0 || index > Config::XeFGMaxInterpolations || countPlus1 < 2 ||
        countPlus1 > Config::XeFGMaxInterpolations + 1 || index >= countPlus1)
        return result;
                      ...
                      void** nativeOut)
{
    ...
    if (!WriteVerified(thunk, replacement, PresentThunkSize))
    {
        ...
        if (!WriteVerified(thunk, expected, PresentThunkSize))
            LOG_ERROR("XeFG pacing: failed to restore thunk at {:#x}", rva);
        return false;
    }
...
inline bool Install(uint8_t* base)
{
    ...
    g_native = reinterpret_cast<PresentFn>(base + NativePresentRva);
