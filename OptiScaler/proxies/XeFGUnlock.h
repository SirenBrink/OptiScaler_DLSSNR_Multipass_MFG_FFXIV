...
class XeFGUnlock
{
  public:
    ...
    static bool Apply(HMODULE module)
    {
        ...
        // Pacing also calls private functions and reads private structures.
        // Matching short instruction sequences alone cannot validate their ABI.
        if (nt->FileHeader.TimeDateStamp != KnownBuildStamp || nt->OptionalHeader.SizeOfImage != KnownSizeOfImage)
        {
            LOG_WARN("XeFG unlock: unsupported provider build {:#010x}/{:#x}; leaving runtime unmodified",
        ...
        // Preflight the entire edit set before changing any instruction.
        for (const auto& patch : patches)
            if (patch.enabled && (!Contains(text, patch.rva, patch.size) ||
                memcmp(base + patch.rva, patch.expected, patch.size) != 0))
            {
                LOG_WARN("XeFG unlock: preflight failed for {}; runtime unchanged", patch.name);
                return false;
            }
        ...
        for (const auto& patch : patches)
        {
            ...
            // Include the attempted write in rollback, even if its read-back fails.
